...
export async function fetchGroqModels(): Promise<string[]> {
  try {
    ...
    const backendUrl = `${(window as any).REACHINBOX_CONFIG?.API_URL || process.env.REACT_APP_API_URL || '/api'}/groq-proxy/models`;
...
export async function generateEmailReply(emailContent: string): Promise<{ subject: string; body: string; confidence: number }> {
  ...
  try {
    ...
    const backendUrl = `${(window as any).REACHINBOX_CONFIG?.API_URL || process.env.REACT_APP_API_URL || '/api'}/groq-proxy/chat`;
